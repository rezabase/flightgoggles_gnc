...
bool Trajectory::ReadFile(const string& filename)
{
  ...
  if (!f)
  {
    //std::cout << "REZA: **** FAILED Trajectory::ReadFile: " << filename << std::endl;
    return false;
  }
  ...
  /*if (traj.size() == 0)
  {
    ...
    traj_pt.position = V3F(0, 0, -3);
    traj_pt.velocity = V3F(0, 0, 0);
    traj_pt.omega = V3F(30, 0, 0);
    traj_pt.attitude = Quaternion<float>::FromEulerYPR(0, 0, 0);
